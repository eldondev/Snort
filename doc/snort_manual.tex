% $Id: snort_manual.tex,v 1.128 2011/06/08 00:33:04 jjordan Exp $
% 
...
Protocol aware flushing of HTTP can be enabled with this option:

\begin{verbatim}
config paf_max: <max-pdu>
\end{verbatim}

where \texttt{<max-pdu>} is between zero (off) and 65280.  This allows Snort to
...
from the "MAIL FROM" command along with all the generated events for that session.
...
Multiple recipients are appended with commas.
...
for that session. Multiple filenames are appended with commas.
...
use for logging email headers. This value along with the \texttt{email\_hdrs\_log\_depth} will determine the 
maximum SMTP sessions that will log the email headers at any given time. When this memcap is 
reached SMTP will stop logging the email headers until memory becomes available.

Max SMTP sessions logging email headers at any given time = \texttt{memcap}/\texttt{email\_hdrs\_log\_depth}
...
It is recommended that user inputs a value that is a multiple of 4.
