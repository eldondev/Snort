        ...
        const u_char *end, HEADER_PTR *header_ptr, sfip_t **true_ip)
{
    ...
    {
        p++;
        sf_unfold_header(p, end-start, unfold_buf, sizeof(unfold_buf), &unfold_size, 0 , 0);
...
static inline const u_char *extractHeaderFieldValues(HI_SESSION *Session,
        ...
        HttpSessionData *hsd, int stream_ins, int *host_name_header)
{
    if (((p - offset) == 0) && ((*p == 'C') || (*p == 'c')))
    {
        ...
        {
            p = extract_http_cookie((p+ HEADER_LENGTH__COOKIE), end, header_ptr, header_field_ptr);
        }
    ...
    else if (((p - offset) == 0) && ((*p == 'x') || (*p == 'X') || (*p == 't') || (*p == 'T')))
    {
        if ( (ServerConf->enable_xff) && hsd )
        {
            if(IsHeaderFieldName(p, end, HEADER_NAME__XFF, HEADER_LENGTH__XFF))
            {
                ...
                p = extract_http_xff(Session, p, start, end, header_ptr, &(hsd->true_ip));
            }
    ...
    else if(((p - offset) == 0) && ((*p == 'H') || (*p == 'h')))
    {
        if(IsHeaderFieldName(p, end, HEADER_NAME__HOSTNAME, HEADER_LENGTH__HOSTNAME))
        {
            ...
            {
                *host_name_header = 1;
                if ( hsd && !stream_ins && (ServerConf->log_hostname))
                {
                    if(!SetLogBuffers(hsd))
                    {
                        p = p + HEADER_LENGTH__HOSTNAME;
                        p = extract_http_hostname(Session, p, start, end, header_ptr, hsd);
                    }
                }
            }
        }
    ...
    */
    ...
    while (hi_util_in_bounds(start, end, p))
    {
        if(*p == '\n')
        {
            ...
            else if ( (p = extractHeaderFieldValues(Session, ServerConf, p, offset, start, end, header_ptr, &header_field_ptr, hsd, stream_ins, &host_name_header)) == end)
            {
                return end;
            }
                ...
                (p = extractHeaderFieldValues(Session, ServerConf, p, offset, start, end, header_ptr, &header_field_ptr, hsd, stream_ins, &host_name_header)) == end)
        {
            return end;
        }
        if ( *p == '\n') continue;
