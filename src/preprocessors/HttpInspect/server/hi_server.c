...
const u_char *extract_http_transfer_encoding(HI_SESSION *Session, HttpSessionData *hsd, const u_char *p, const u_char *start,
...
/*    if( ServerConf->server_flow_depth && ((end - ptr) > ServerConf->server_flow_depth) )
    ...
    if (!(sd->resp_state.last_pkt_contlen))
    {
        ...
        {
            if(sd->resp_state.last_pkt_chunked && CheckChunkEncoding(Session, start, end, &post_end, (u_char *)HttpDecodeBuf.data, sizeof(HttpDecodeBuf.data),
        ...
        const u_char *end, URI_PTR *result)
{
    ...
    if(!(sd->resp_state.last_pkt_contlen))
    {
        if(sd->resp_state.last_pkt_chunked && CheckChunkEncoding(Session, start, end, NULL, compr_buffer, compr_avail,
                    sd->resp_state.last_chunk_size, &chunk_size, &chunk_read ) == 1)
        {
            sd->resp_state.last_chunk_size = chunk_size;
