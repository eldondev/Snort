...
static int SIP_processRequest(SIPMsg *, SIP_DialogData *, SIP_DialogList *);
static int SIP_processInvite(SIPMsg *, SIP_DialogData *, SIP_DialogList *);
static int SIP_processACK(SIPMsg *, SIP_DialogData *, SIP_DialogList *);
static int SIP_processResponse(SIPMsg *, SIP_DialogData *, SIP_DialogList *);
static int SIP_ignoreChannels( SIP_DialogData *);
...
static int SIP_processRequest(SIPMsg *sipMsg, SIP_DialogData *dialog, SIP_DialogList *dList)
{
    ...
    switch (methodFlag)
	{
    	...
    	case SIP_METHOD_ACK:
    		...
    		SIP_processACK(sipMsg, dialog, dList);
 ...
 *  SIP_FAILURE:
 ********************************************************************/
static int SIP_processACK(SIPMsg *sipMsg, SIP_DialogData *dialog, SIP_DialogList *dList)
{
	if (NULL == dialog)
		return SIP_FAILURE;
 ...
 *
 ********************************************************************/
static int SIP_ignoreChannels( SIP_DialogData *dialog)
{
...
int SIP_updateDialog(SIPMsg *sipMsg, SIP_DialogList *dList)
{
   	...
   	if (sipMsg->status_code == 0)
   		ret = SIP_processRequest(sipMsg, dialog, dList);
   	else if (sipMsg->status_code > 0)
   		ret = SIP_processResponse(sipMsg, dialog, dList);
   	else
   		ret = SIP_FAILURE;
